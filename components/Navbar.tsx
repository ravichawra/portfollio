      ...
      <div className="max-w-container mx-auto px-gutter flex items-center justify-between h-16">
        ...
        <nav className="hidden md:flex items-center gap-8" aria-label="Main navigation">
          {backLink ? (
            <Link
              href={backLink.href}
              className="font-mono text-micro uppercase tracking-widest text-text-muted hover:text-lime transition-colors duration-200 flex items-center gap-2"
            >
              {backLink.label}
            </Link>
          ) : (
            ...
            <nav className="flex flex-col px-gutter py-6 gap-6" aria-label="Mobile navigation">
              {NAV_LINKS.map((link, i) => (
                <motion.a
                  ...
                  transition={{ delay: i * 0.06, ease: [0.0, 0.0, 0.2, 1] as [number, number, number, number] }}
              ...
              <motion.a
                ...
                initial={{ x: -20, opacity: 0 }}
                animate={{ x: 0, opacity: 1 }}
                transition={{ delay: NAV_LINKS.length * 0.06, ease: [0.0, 0.0, 0.2, 1] as [number, number, number, number] }}
                className="bg-lime text-on-primary font-mono text-label uppercase tracking-widest px-6 py-3 font-bold text-center w-full block"
                onClick={() => setMenuOpen(false)}
              >
                Book a Call
              </motion.a>
